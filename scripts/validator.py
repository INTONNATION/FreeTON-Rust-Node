...
def cli_get_recover_amount (elector_addr: str, msig_addr_hex: str):
    recover_amount=subprocess.check_output('tonos-cli run %s compute_returned_stake \"{\\"wallet_addr\\":\\"%s\\"}\" --abi %s/Elector.abi.json | grep value0 | awk \'{print $2}\' | tr -d \'"\' | printf %%d' % (elector_addr, msig_addr_hex, configs_dir), encoding = 'utf-8', shell=True)
...
def cli_submit_transaction (msig_addr: str, elector_addr_hex: str, value: str, boc: str):
    trx=subprocess.check_output('tonos-cli call %s submitTransaction \
            ...
            --sign %s/msig.keys.json' % (msig_addr, elector_addr_hex, value, boc, configs_dir, configs_dir), encoding = 'utf-8', shell=True)
    logging.info(trx)
    return trx
...
def console_create_elector_request():
    ...
    elections_end_before=subprocess.check_output('cat global_config_15_raw | grep elections_end_before | awk \'{print $2}\' | tr -d \',\'', encoding = 'utf-8', shell=True)
    elections_start_before=subprocess.check_output('cat global_config_15_raw | grep "elections_start_before" | awk \'{print $2}\' | tr -d \',\'', encoding = 'utf-8', shell=True)
    stake_held_for=subprocess.check_output('cat global_config_15_raw | grep "stake_held_for" | awk \'{print $2}\' | tr -d \',\'', encoding = 'utf-8', shell=True)
    validators_elected_for=subprocess.check_output('cat global_config_15_raw | grep "validators_elected_for" | awk \'{print $2}\' | tr -d \',\'', encoding = 'utf-8', shell=True)
    election_start=cli_get_active_election_id(elector_addr)
    election_stop=(int(election_start) + 1000 +  int(elections_start_before) + int(elections_end_before) + int(stake_held_for) + int(validators_elected_for))
    request=subprocess.check_output('console -C %s/console.json -c "election-bid %s %s"' % (configs_dir, election_start, election_stop), encoding = 'utf-8', shell=True)
...
def check_validator_balance (msig_addr_hex: str):
    balance=subprocess.check_output('tonos-cli account %s | grep balance | awk \'{print $2}\'' % (msig_addr), encoding = 'utf-8', shell=True)
